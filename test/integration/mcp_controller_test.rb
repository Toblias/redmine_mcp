...
class McpControllerTest < ActionDispatch::IntegrationTest
  ...
  def setup
    ...
    # Enable REST API (required for API key auth to work)
    Setting.rest_api_enabled = '1'

    # Generate API key for user if not present
    # In Redmine, Token with action 'api' stores the API key
    @api_key = @user.api_key
    unless @api_key.present?
      # Delete any existing api tokens and create new one
      Token.where(user_id: @user.id, action: 'api').destroy_all
  ...
  test 'sse endpoint returns 503 when plugin disabled' do
    ...
    get '/mcp', headers: { 'X-Redmine-API-Key' => @api_key }
  ...
  test 'message endpoint returns error for invalid JSON' do
    ...
    # Rails middleware intercepts malformed JSON and returns 400 before our controller runs.
    # This is acceptable behavior - invalid requests are rejected at the framework level.
